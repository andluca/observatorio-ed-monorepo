...
import { Plus, Edit, Trash2, FileText, User, BookOpen, FileTextIcon, Users } from "lucide-react";
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center gap-3">
            {user.role === "ADMIN" && (
              <Link
                ...
                className="inline-flex items-center gap-2 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 font-medium px-4 py-2 rounded-md transition-colors shadow-sm"
              >
                <Users size={18} />
                    ...
                    <tr key={post.id} className="hover:bg-gray-50 transition-colors group">
                          ...
                          <div className="flex items-center">
                            {post.type === 'ARTICLE' ? (
                              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium bg-blue-50 text-blue-700 border border-blue-100">
                                <BookOpen size={12} /> Artigo Acadêmico
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium bg-purple-50 text-purple-700 border border-purple-100">
                                <FileTextIcon size={12} /> Texto / Notícia
                              </span>
                            )}
                            {post.featured && (
                                <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-[#FFC700] text-black">
                                    DESTAQUE
                                </span>
                            )}
                          </div>
                      ...
                      <td className="px-6 py-4 text-right whitespace-nowrap">
                        <div className="flex justify-end gap-2">
                          ...
                          {/* Exclusão */}
                          <button className="p-2 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-md border border-transparent hover:border-red-100 transition-all" title="Excluir">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
            ...
            <p className="text-3xl font-bold text-gray-400">{stats.draftPosts}</p>
          </div>
        </div>
